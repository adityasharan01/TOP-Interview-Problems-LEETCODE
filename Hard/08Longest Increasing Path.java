...
Hint2:Due to the strictly increasing condition, there cannot be any cycles.
Hint2:Question yourself on what will be the longest increasing path if I start from each cell (consider each cell as a starting point and traverse)
...
Time Complexity
A constant amount of operations are done per matrix cell location. So for a matrix of size n*n, time complexity is \mathcal{O}(n^2)O(n 
2
 )

Space Complexity
A dp table of size equal to matrix is used. Therefore space complexity is \mathcal{O}(n^2)O(n 
2
 )
